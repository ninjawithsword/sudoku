...
int solver() {
  ...
  for(i = 0; i < num_of_solutions[0]; i++) { // one for each line
    copy_line(0, &potential[i * 10 + offset(0)]);
    for(j = 0; j < num_of_solutions[1]; j++) {
      copy_line(1, &potential[j * 10 + offset(1)]);
      for(k = 0; k < num_of_solutions[2]; k++) {
  	copy_line(2, &potential[k * 10 + offset(2)]);
  	for(l = 0; l < num_of_solutions[3]; l++) {
  	  copy_line(3, &potential[l * 10 + offset(3)]);
  	  for(m = 0; m < num_of_solutions[4]; m++) {
  	    copy_line(4, &potential[m * 10 + offset(4)]);
  	    for(n = 0; n < num_of_solutions[5]; n++) {
  	      copy_line(5, &potential[n * 10 + offset(5)]);
  	      for(o = 0; o < num_of_solutions[6]; o++) {
  		copy_line(6, &potential[o * 10 + offset(6)]);
  		for(p = 0; p < num_of_solutions[7]; p++) {
  		  copy_line(7, &potential[p * 10 + offset(7)]);
  		  for(q = 0; q < num_of_solutions[8]; q++) {
  		    copy_line(8, &potential[q * 10 + offset(8)]);
